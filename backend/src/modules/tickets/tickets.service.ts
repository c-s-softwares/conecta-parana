...
export class TicketsService {
  ...
  async create(
    ...
    if (!user || !user.cityId) {
      throw new BadRequestException(apiError(TICKET_ERRORS.TICKET_USER_WITHOUT_CITY));
    }
    ...
    if (!allowedTypes.includes(dto.type)) {
      throw new BadRequestException(apiError(TICKET_ERRORS.TICKET_INVALID_TYPE));
    }
    ...
    if (dto.photoIds && dto.photoIds.length > 0) {
      ...
      if (photos.length !== dto.photoIds.length) {
        throw new BadRequestException(apiError(TICKET_ERRORS.TICKET_PHOTO_NOT_FOUND));
      }
